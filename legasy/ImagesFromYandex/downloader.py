...
DRIVER_NAME_TO_CLASS: dict[str, Type[Driver]] = {
...
class YandexImagesDownloader():
    ...
    def download_images_by_page(self, keyword, page, imgs_count,
                                sub_directory, search_by_image: bool) -> PageResult:
        ...
        self.check_captcha_and_get(YandexImagesDownloader.MAIN_URL, search_by_image,
    ...
    def download_images_by_keyword(self, keyword, search_by_image: bool, sub_directory="") -> KeywordResult:
    ...
    def download_images(self, keywords: list[str], search_by_image: bool) -> DownloaderResult:
        ...
        for keyword in keywords:
            ...
            keyword_result = self.download_images_by_keyword(
                keyword, search_by_image, sub_directory=keyword)
    ...
    def check_captcha_and_get(self, url: str, search_by_image: bool, params=None):
        """Checking for captcha on url and get url after that.
        If there is captcha, you have to type it in input() or quit."""
        url_with_params = '<YOUR_URL_TO_SIMILAR_IMAGES_IN_YANDEX>, ex.: ' \
                          'https://yandex.ru/images/search?cbir_id=765407%2F2cWt348WdF0cAaeZj_qTOQ2173&cbir_page=similar&lr=213&rpt=imageview&source-serpid=8yLFPsANSltGTqkjal5WZg&url=https%3A%2F%2Favatars.mds.yandex.net%2Fget-images-cbir%2F765407%2F2cWt348WdF0cAaeZj_qTOQ2173%2Forig' \
            if search_by_image else f"{url}?{urlencode(params)}"

        del self.driver.requests
        self.driver.get(url_with_params)

        logging.warning(f"Please, type something to continue")
        _ = input()
